...
internal class Program
{
    ...
    private static void printShoppingCart(Store atb)
    {
        Console.WriteLine("Your chosen cars:");
        for (int i = 0; i < atb.ShoppingList.Count; i++)
        {
            Console.WriteLine("Car: #" + i + " " + atb.ShoppingList[i]);
        }
    }

    private static void printInventory(Store atb)
    {
        for(int i = 0; i < atb.CarList.Count; i++) 
        {
            Console.WriteLine("Car: #"+ i + " "+ atb.CarList[i]);
        }
    }
